  ...
  async function generateFeed(localePath) {
    ...
    const rss = new RSS({
      ...
      custom_elements: [
        ...
        { 'itunes:category': siteConfig.itunes.category },
        { 'category': siteConfig.itunes.category },
        {
          'itunes:image': {
            _attr: {
    ...
    items.forEach(post => {
      ...
      rss.item({
        ...
        date: post.published,
        custom_elements: [{
          'itunes:duration': post.enclosure && post.enclosure.duration
        }]
      })
    })
